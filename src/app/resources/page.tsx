...
import { FileText, Download, ExternalLink, Newspaper, Megaphone, Award, BookOpen, Lightbulb, Zap, Handshake, Cog, Bot, Building2, Landmark, Trophy, type LucideIcon } from "lucide-react";
...
const BROCHURES = [
  { name: "VSI Course Catalog 2024", desc: "Complete guide to all certificate, diploma and vocational programs.", size: "2.4 MB", color: "#1565c0", Icon: BookOpen },
  { name: "CAD Engineering Programs", desc: "Detailed syllabus and fee structure for all CAD courses.", size: "1.8 MB", color: "#0d7a55", Icon: Cog },
  { name: "Data Science & AI Programs", desc: "Curriculum overview for data analytics, AI/ML and digital marketing.", size: "1.5 MB", color: "#6a1b9a", Icon: Bot },
  { name: "Placement Brochure", desc: "Placement statistics, recruiter list and salary data.", size: "3.1 MB", color: "#b8520a", Icon: Building2 },
  { name: "Centre Profiles", desc: "Location-wise infrastructure, faculty and lab details.", size: "2.0 MB", color: "#c62828", Icon: Landmark },
];
...
function AdsThumbnail({ color, tag, index }: { color: string; tag: string; index: number }) {
  const icons = [Megaphone, Lightbulb, Zap, Handshake];
  const AdIcon = icons[index % icons.length];
  return (
    <div style={{
      ...
      <div style={{
        ...
        <AdIcon size={34} strokeWidth={1.25} color="#fff" opacity={0.45} aria-hidden="true" />
      </div>
      <div style={{
        position: "absolute", bottom: 10, left: 14,
        fontFamily: "var(--mono)", fontSize: "var(--text-xs)", fontWeight: 700, letterSpacing: "var(--tr-mono)",
...
function BrochureThumbnail({ color, Icon: Mark }: { color: string; Icon: LucideIcon }) {
  return (
    <div style={{
      width: 72, height: 88, borderRadius: "var(--r-sm)", flexShrink: 0,
      ...
      <div style={{ position: "absolute", bottom: 12, left: 8, right: 8 }}>
        {[0, 1, 2].map((j) => (
          <div key={j} style={{ height: 1.5, background: `${color}44`, marginBottom: 4, borderRadius: "var(--r-sm)", width: j === 2 ? "60%" : "100%" }} />
        ))}
      </div>
      <Mark size={22} strokeWidth={1.4} color="#fff" opacity={0.8} style={{ zIndex: 1 }} aria-hidden="true" />
    </div>
  );
}
                ...
                <AnimateIn key={i} animation="slideUp" delay={i * 0.06}>
                  <article className="course-row" style={{
                    ...
                    <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
                      ...
                      <h3 style={{ fontFamily: "var(--serif)", fontWeight: 700, fontSize: "var(--text-md)", color: "var(--text)", letterSpacing: "var(--tr-heading)", lineHeight: 1.3 }}>{item.t}</h3>
                      <p style={{ fontFamily: "var(--body)", fontSize: "var(--text-sm)", color: "var(--text-muted)", lineHeight: 1.65 }}>{item.s}</p>
                    </div>
                  </article>
                </AnimateIn>
              ))}
              ...
              {ADS.map((ad, i) => (
                <AnimateIn key={i} animation="slideUp" delay={i * 0.06}>
                  <article className="course-row" style={{
                    ...
                    <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
                      <span style={{ fontFamily: "var(--mono)", fontSize: "var(--text-xs)", letterSpacing: "var(--tr-mono)", color: "var(--accent-gold)", textTransform: "uppercase", fontWeight: 500 }}>{ad.date}</span>
                      <h3 style={{ fontFamily: "var(--serif)", fontWeight: 700, fontSize: "var(--text-md)", color: "var(--text)", letterSpacing: "var(--tr-heading)", lineHeight: 1.3 }}>{ad.title}</h3>
                      <p style={{ fontFamily: "var(--body)", fontSize: "var(--text-sm)", color: "var(--text-muted)", lineHeight: 1.65 }}>{ad.desc}</p>
                    </div>
                  </article>
                </AnimateIn>
              ))}
                  ...
                  <div style={{
                    ...
                    <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 10, flex: 1, textAlign: "center" }}>
                      ...
                      <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: "auto", paddingTop: 6 }}>
                        <span style={{ fontFamily: "var(--mono)", fontSize: "var(--text-xs)", color: "var(--accent-gold)", letterSpacing: "var(--tr-caps)" }}>{award.y}</span>
                        <span style={{ fontFamily: "var(--mono)", fontSize: "var(--text-xs)", color: "var(--text-muted)", letterSpacing: "var(--tr-caps)" }}>{award.by}</span>
                      </div>
                    </div>
                  </div>
                </AnimateIn>
              ...
              {BROCHURES.map((brochure, i) => (
                <AnimateIn key={i} animation="slideUp" delay={i * 0.05}>
                  <div className="course-row" style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <h3 style={{ fontFamily: "var(--serif)", fontWeight: 700, fontSize: "var(--text-base)", color: "var(--text)", letterSpacing: "var(--tr-body)" }}>{brochure.name}</h3>
                      <p style={{ fontFamily: "var(--body)", fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 4, lineHeight: 1.5 }}>{brochure.desc}</p>
                      <span style={{ fontFamily: "var(--mono)", fontSize: "var(--text-xs)", color: "var(--text-muted)", display: "inline-block", marginTop: 6 }}>PDF · {brochure.size}</span>
                    </div>
                    <button className="btn-secondary" style={{ padding: "8px 16px", fontSize: "var(--text-xs)", display: "flex", alignItems: "center", gap: 5, flexShrink: 0 }}>
